...
public class AdapterRcv extends RecyclerView.Adapter<AdapterRcv.PostViewHolder> {
    ...
    private CommentsScreen commentsScreen;
    ...
    public AdapterRcv(Activity context, List<Data> mList, List<Users> usersList1,CommentsScreen commentsScreen) {
        this.mList = mList;
        this.context = context;
        this.usersList1 = usersList1;
        this.commentsScreen=commentsScreen;
    }
...
//                holder.setTime(date);
        ...
        firebaseFirestore.collection("Posts/" + postId + "/Likes").document(currentUserId).addSnapshotListener(new EventListener<DocumentSnapshot>() {
            ...
            public void onEvent(@Nullable DocumentSnapshot value, @Nullable FirebaseFirestoreException error) {
                if (error == null) {
                    if (value.exists()) {
                        holder.likeImg.setImageDrawable(context.getDrawable(R.drawable.redlove));
                    } else {
...
//                bundle.putString("postid", postId);
                commentsScreen.moveToCommentFrag(post);
//                navController.navigate(R.id.action_homeFragment_to_commentFragment, bundle);
    ...
    public interface CommentsScreen {
        void moveToCommentFrag(Data post);
    }
}
